...
Description 是 agent 判断页面相关性的快速信号。必须遵循以下规范来保证 vault 的检索效能：

**基本规范**
- **长度**：20 字以上，描述清楚即可，不硬限句数
- **不包含**：换行符、列表符号、纯网址
- **目标**：让 agent 能快速判断页面相关性，传达"是什么 + 为什么有用"
- **禁止模式**：`项目：`, `description:`, `---`, 纯网址、纯 emoji

**按内容类型的风格规则**

| 类型 | 风格 | 示例 |
|------|------|------|
| **GitHub 项目** | 值导向。说明这个工具能帮什么、适用什么场景 | `"LLM 驱动的代码补全工具。支持实时建议，与 IDE 集成，加速长期项目开发。"` |
| **论文/深度文章** | 带观点。说明技术是什么、关键创新、对你的实际意义 | `"提出了分布式 Agent 调度的新范式。核心是事件驱动而非轮询，对实时风险管理 Agent 意义重大。"` |
| **推文/推荐** | 严肃总结。提取技术/观点核心，去掉情绪化语言 | `"探讨了 Agent planning 的限制。指出 function calling 不足以解决 reasoning，需要更深层的搜索。"` |

**生成规则（ingest 时的检查清单）**

1. **Web Clipper 抓取的描述**：通常是正文开头 200 字，需要重新总结
   - ❌ 不要直接用 web clipper 截断的内容
   - ✓ 读一遍正文，用上表风格规则生成新 description

2. **GitHub 项目**：用 README 的核心功能 + 你的应用场景
   - ❌ 不要用项目的技术栈列表
   - ✓ 提取"为什么用"而不是"怎么用"

3. **推文/推荐**：总结原作的观点 + 关键启发
   - ❌ 不要保留推文的口语化、情绪、@mention
   - ✓ 提炼核心，用第三人称学术语气

**质量检查（frontmatter-validator 执行）**

- 字数 < 20：❌ FAIL（描述过短，无法作为检索信号）
- 包含 `项目：`、`description:`、`---`、纯网址等占位符模式：❌ FAIL（垃圾信息，需重新生成）
- 字数 > 200：⚠ WARN（可能是直接粘贴的正文，考虑精简）
...
| `docs/solutions/` | 已解决问题的文档库（bug fixes、best practices、工作流决策），按类别组织，包含 YAML frontmatter（problem_type、module、tags）。维护 vault、生成 frontmatter、修复格式问题时先查阅 |
